...
**Style sources:** Reads `.windsurf/templates/GOAL-general.md` as a comparison baseline (not a generation template). Then reads `handTyped(latest)/` if it exists, otherwise scans all completed phase folders. Identifies where the user's style deviates from the universal conventions.
...
**What it reads:**
- `.windsurf/templates/Dependency-template.md`, `GUIDE-template.md`, `FLOW-template.md` — exact structures to follow
