...
8a. **Synthesis Checklist** — After reading all files, produce these specific summary tables (used by ARCHITECTURE.md sections 3-7):
    ...
    Cross-check total files analyzed against the Glob count from step 6. If any files are missing, go back and read them.
...
26. **Read `.windsurf/rules/csharp-conventions.md`** before writing any .cs files below. Then create `LEARN/{PROJECT}/phase-All/` with the shared foundation scripts that ALL phases depend on:
