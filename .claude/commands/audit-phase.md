...
All rules referenced in the violation checks and mistake checks below are defined in [C# Conventions](.claude/instructions/csharp-conventions.md). Use that file as the authoritative source for every rule.

## Skills Reference

Load the relevant skill when auditing domain-specific areas:

- **[unity-testing](.claude/skills/unity-testing.md)** — when verifying test completeness (§9), Test.md format, DEBUG_Check patterns, mock strategy
- **[unity-scene-setup](.claude/skills/unity-scene-setup.md)** — when verifying scene setup docs, layer/tag correctness, lighting/material references
- **[unity-audio](.claude/skills/unity-audio.md)** — when auditing sound system usage (pool patterns, distance culling, LoopingSoundPlayer wiring)
- **[unity-animation](.claude/skills/unity-animation.md)** — when verifying AnimParamType usage (no raw strings), animation-to-gameplay sync timing, motion patterns
- **[unity-prefab-hierarchy](.claude/skills/unity-prefab-hierarchy.md)** — when verifying prefab GO structure, ViewModel/WorldModel splits, collider zone naming, connection point patterns
- **[unity-save-load](.claude/skills/unity-save-load.md)** — when auditing save/load completeness, ISaveable contracts, SaveData purity, round-trip integrity
- **[unity-fsm](.claude/skills/unity-fsm.md)** — when auditing state machines, IState implementations, Exit() cleanup, transition correctness
- **[unity-networking](.claude/skills/unity-networking.md)** — when auditing network authority, RPC patterns, SyncVar usage, client/server separation
- **[unity-day-night](.claude/skills/unity-day-night.md)** — when auditing time system, day/night transitions, IDaytimeSensitive implementations
- **[unity-camera](.claude/skills/unity-camera.md)** — when auditing camera rigs, Cinemachine setup, screen shake, FOV transitions
- **[unity-dialogue](.claude/skills/unity-dialogue.md)** — when auditing dialogue systems, NPC conversations, third-party dialogue integration
- **[unity-ai-navigation](.claude/skills/unity-ai-navigation.md)** — when auditing NPC pathfinding, NavMesh agent config, FSM state transitions, patrol/chase patterns
- **[unity-quest](.claude/skills/unity-quest.md)** — when auditing quest/objective tracking, QuestSO definitions, completion callbacks, quest save/load
- **[unity-procedural-gen](.claude/skills/unity-procedural-gen.md)** — when auditing procedural generation, seed reproducibility, chunk loading, biome assignment
- **[unity-input](.claude/skills/unity-input.md)** — when auditing input handling, InputActions configuration, rebinding persistence, action map switching
- **[unity-physics](.claude/skills/unity-physics.md)** — when auditing physics setup, Rigidbody config, joint connections, ragdoll activation, raycast layers
- **[unity-inventory](.claude/skills/unity-inventory.md)** — when auditing inventory slot logic, stacking rules, drag-drop wiring, equipment constraints
- **[unity-grid-building](.claude/skills/unity-grid-building.md)** — when auditing building placement, grid validation, ghost material setup, footprint rotation
...
6. Scan every `.cs` in `LEARN/{PROJECT}/phase-{PHASE}/` for violations of [C# Conventions](.claude/instructions/csharp-conventions.md). Key items to check:
...
7. Specifically check for each of these mistakes (from NewAgent.md — full rules in [C# Conventions](.claude/instructions/csharp-conventions.md)):
