---
description: "Implement .claude/ framework improvements by deep-reading MAIN-SOURCE/ projects and applying changes: create new skills from real source patterns, update existing skills with genre variants, modify conventions/templates/commands. Works from gap reports (audit-required-todo.md), /audit-framework output, or user-specified changes. Use when: after /audit-framework produces a gap report, or when you know exactly what framework changes are needed."
---
...
> REAL code patterns, and implements all approved changes to `.claude/` files — creating new skills,
> updating existing skills with genre variants, modifying conventions, templates, and prompts.

> **This command ONLY modifies `.claude/` files** — never touches `MAIN-SOURCE/` or `LEARN/` project content.
...
- "Where is the gap report / TODO list?"
  - Default: `.claude/audit-required-todo.md`
...
- **Target file** — which `.claude/` file to create or modify
...
- `.claude/skills/` directory with existing skills (format reference)
- `CLAUDE.md` (registration target)
- `.claude/MANUAL.md` (registration target)
...
Read ALL `.claude/skills/*/SKILL.md` files fully. Extract and memorize the exact format:
...
Create `.claude/skills/{skill-name}/SKILL.md`. Follow this complete template:
...
| 7 | No project-specific names in any `.claude/` file prose | |
...
If `.claude/audit-required-todo.md` exists:
...
10. **Every TODO maps to a file change** — the output of this command is modified `.claude/` files. If a TODO item can't be translated to a specific file edit, it's not actionable and should be reported back to the user for clarification.