...
All naming, capitalization, class roles, decoupling, mandatory patterns, pitfalls, and common mistakes are defined in [C# Conventions](.claude/instructions/csharp-conventions.md). Every script generated below MUST follow these conventions — no exceptions.

## Skills Reference

Domain-specific knowledge lives in `.claude/skills/`. Load the relevant skill when working on that domain:

- **[unity-testing](.claude/skills/unity-testing.md)** — when generating `5-Tests/`, `DEBUG_Check`, `Test.md` files, or manual test guides
- **[unity-scene-setup](.claude/skills/unity-scene-setup.md)** — when generating `WORLD.md` sections, scene setup instructions in GUIDE.md, or layer/lighting/material docs
- **[unity-audio](.claude/skills/unity-audio.md)** — when the phase includes SoundManager, SoundDefinition, LoopingSoundPlayer, or any audio playback
- **[unity-animation](.claude/skills/unity-animation.md)** — when the phase includes AnimatorControllers, AnimParamType enum values, code-driven rotation, or shader-driven motion
- **[unity-prefab-hierarchy](.claude/skills/unity-prefab-hierarchy.md)** — when structuring prefab GameObjects, ViewModel/WorldModel splits, collider zones, connection points, or inheritance chains
- **[unity-save-load](.claude/skills/unity-save-load.md)** — when the phase includes ISaveable, SaveData classes, SaveManager, or any save/load persistence
- **[unity-fsm](.claude/skills/unity-fsm.md)** — when the phase includes IState, StateMachine, AI states, or state-driven gameplay
- **[unity-day-night](.claude/skills/unity-day-night.md)** — when the phase includes DayNightCycle, time progression, or day/night-sensitive behavior
- **[unity-ai-navigation](.claude/skills/unity-ai-navigation.md)** — when the phase includes NavMeshAgent, patrol waypoints, AI pathfinding, or A*Pathfinding
- **[unity-networking](.claude/skills/unity-networking.md)** — when the phase includes networked RPCs, SyncVars, client/server authority, or multiplayer
- **[unity-quest](.claude/skills/unity-quest.md)** — when the phase includes quest definitions, objective tracking, or quest UI
- **[unity-procedural-gen](.claude/skills/unity-procedural-gen.md)** — when the phase includes procedural terrain, chunk generation, seeded random, or Perlin noise
- **[unity-camera](.claude/skills/unity-camera.md)** — when the phase includes Cinemachine, manual camera rigs, screen shake, or FOV transitions
- **[unity-dialogue](.claude/skills/unity-dialogue.md)** — when the phase includes NPC dialogue, conversation trees, YarnSpinner, Ink, or PixelCrushers
- **[unity-input](.claude/skills/unity-input.md)** — when the phase includes player input handling, InputActions, rebinding UI, action maps, or context switching
- **[unity-physics](.claude/skills/unity-physics.md)** — when the phase includes Rigidbody forces, joints, ragdoll, physics-based grab, raycasting, or trigger zones
- **[unity-inventory](.claude/skills/unity-inventory.md)** — when the phase includes item slots, stacking, drag-drop, equipment, hotbar, or container transfer
- **[unity-grid-building](.claude/skills/unity-grid-building.md)** — when the phase includes grid placement, ghost preview, building validation, snap-to-grid, or free-form placement
...
18. Create `GUIDE.md` — follow `.claude/templates/GUIDE-template.md` for section order and format. Beginner-friendly, conversational voice. Must include ALL of:
...
19. Create `FLOW.md` — follow `.claude/templates/FLOW-template.md` for section order and format. System connections. Must include ALL of:
