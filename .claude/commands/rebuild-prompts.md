---
description: "Deep-analyse a project's LEARN/{PROJECT}/ folder (all phases, scripts, docs) and verify/update .claude/commands/*.md to match reality. Use when: after completing multiple phases, when commands feel stale, when audit flags wrong command instructions."
---
...
> **This command does NOT modify project-specific files** (`LEARN/{PROJECT}/GOAL.md`, `LEARN/{PROJECT}/NewAgent.md`, phase scripts). It only updates the `.claude/commands/*.md` workflow files.

> **Scope:** Every `.md` in `.claude/commands/` is a target — `init`, `build-phase`, `audit-phase`, `decouple-check`, `cross-phase-mod`, `post-delivery-sync`, `update-goal-from-handtyped`, `add-system-to-phase`, `refactor-interface`, `evolve-shared-infra`, `rebuild-templates`, and this file itself.
...
Read `.claude/MANUAL.md`. Update it to reflect any command changes:
...
- **Commands listing in workspace structure** — verify every command `.md` is listed
- **Architecture Conventions summary** — if any command fix changed what conventions are enforced (e.g., new exceptions to rules, new mandatory patterns), update the conventions summary to match
