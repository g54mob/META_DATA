...
>
> **What are slash commands?** They're automated workflows. Type `/` in Claude Code, pick a command from the dropdown, and the agent follows the steps defined in a `.md` file. You don't need to know what's inside the command file — just when to use each command.

---

## How to Trigger

In VS Code's Claude Code panel, type `/` followed by the command name. A dropdown appears — select it and press Enter. The agent reads the `.md` file and executes every step automatically.
...
| `/rebuild-prompts` | Audit command files against real code patterns | After 3+ phases | Prompts produce false positives || `/audit-framework` | Multi-project framework gap analysis | Before adding new genre, after 2+ projects | Framework feels insufficient for new domain |
...
**What are skills?** Skills are `.md` files in `.claude/skills/*/SKILL.md` that contain domain-specific knowledge. They are NOT slash commands — you never type `/unity-audio`. Instead, Claude Code **automatically loads** a skill when the current task matches the skill's description (e.g., implementing sound → loads `unity-audio`, building a scene → loads `unity-scene-setup`).

**How they work:** Each skill has YAML frontmatter with a `description` field. Claude Code reads descriptions first (lightweight). When a task matches, it reads the full skill body to get detailed patterns, code examples, and rules. Skills are progressive — they don't bloat every conversation, only the relevant ones load.
...
- `.claude/templates/` — ALL 15 template files (ARCHITECTURE, PhaseMap, StructureMap, Estimate, portability maps, GameStateSoFar, GUIDE, etc.)
- `.claude/templates/GOAL-general.md` — universal architecture rules template
...
**Style sources:** Reads `.claude/templates/GOAL-general.md` as a comparison baseline (not a generation template). Then reads `handTyped(latest)/` if it exists, otherwise scans all completed phase folders. Identifies where the user's style deviates from the universal conventions.
...
**What it reads:**
- `.claude/templates/Dependency-template.md`, `GUIDE-template.md`, `FLOW-template.md` — exact structures to follow
...
**What is this?** Prompt files (`.claude/commands/*.md`) tell the agent what to do. But as your code evolves, prompts can become stale — they might instruct patterns you've abandoned or miss patterns you've adopted. This command reads your actual code and audits every prompt for accuracy.
...
- The entire `.claude/` framework: CLAUDE.md, all skills, all templates, all prompts, all conventions
...
Creates new skills, updates templates/commands/conventions, updates Manual.md and CLAUDE.md to register new additions.

**Key behavior:** STOPS after gap report. Asks "Which items should I implement?" Waits for your confirmation before modifying any `.claude/` files.
...
**What it reads:**
- Gap report file (default: `.claude/audit-required-todo.md`) or user instructions
- `MAIN-SOURCE/*/` — ALL `.cs` files for relevant domains
- ALL existing `.claude/skills/*/SKILL.md` — format reference
- `.claude/CLAUDE.md`, `Manual.md` — registration targets
...
7. Registers all new skills in CLAUDE.md, Manual.md, and prompt skill-reference sections
8. Marks completed items in the gap report file

**Change types it handles:**
- `create-skill` — new `.claude/skills/{name}/SKILL.md` with full source extraction
...
- **In-depth analysis is MANDATORY** — `CLAUDE.md` enforces thorough analysis before every response. This applies to ALL Claude Code chats and ALL prompt commands without exception.
...
- **Only files in `.claude/commands/` become slash commands** — everything else in `.claude/` is regular files.
- **`.claude/CLAUDE.md` is always loaded** — architecture conventions summary every Claude Code response already knows.
- **`.claude/templates/` holds universal templates** — copied + customized per project by `/init`.
...
- **Skills auto-load** — `.claude/skills/*/SKILL.md` files load automatically when the task matches their description. You never invoke them manually.
