...
>
> **What are slash commands?** They're automated workflows. Type `/` in Claude Code CLI or the VS Code extension, pick a command, and the agent follows the steps defined in a `.md` file under `.claude/commands/`. You don't need to know what's inside the command file — just when to use each command.

---

## How to Trigger

In Claude Code CLI or the VS Code extension chat, type `/` followed by the command name. The agent reads the command file and executes every step automatically.
...
ROOT/
├── .claude/
│   ├── commands/                     ← slash commands (type /command-name)
│   ├── skills/                       ← domain knowledge (referenced by commands)
│   ├── templates/                    ← doc generation formats
│   ├── instructions/                 ← file-type conventions (C# rules)
│   ├── WORKSPACE-REG.md             ← project registry (quick-lookup: scale, genre, skills, status)
│   ├── MANUAL.md                    ← this file
│   ├── ROADMAP.md                   ← framework maturity & future plans
│   └── v1.1.ver                     ← version marker
...
- `.claude/templates/` — ALL 15 template files (ARCHITECTURE, PhaseMap, StructureMap, Estimate, portability maps, GameStateSoFar, etc.)
...
**What is this?** Command files (`.claude/commands/*.md`) tell the agent what to do. But as your code evolves, commands can become stale — they might instruct patterns you've abandoned or miss patterns you've adopted. This command reads your actual code and audits every command for accuracy.
...
**What is this?** A meta-audit of the entire `.claude/` framework. Instead of checking one project's code against one command, this scans ALL projects in the workspace and asks: "Does this framework handle every Unity3D genre well?" It finds missing skills, template gaps, command blind spots, and convention holes that only surface when you work across horror, survival, co-op, scheduling, and other diverse game types.
...
- The entire `.claude/` framework: CLAUDE.md, all skills, all templates, all commands, all conventions
...
- ALL existing `.claude/skills/*.md` — format reference
...
- `create-skill` — new `.claude/skills/{name}.md` with full source extraction
...
- **Only files in `.claude/commands/` become slash commands** — everything else is regular files.
...
- **Skills load when referenced** — `.claude/skills/*.md` files are referenced by commands when the task matches their description. You never invoke them manually.
