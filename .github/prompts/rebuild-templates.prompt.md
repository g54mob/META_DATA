---
mode: agent
description: "Deep-analyse a project's LEARN/{PROJECT}/ folder (all phases, scripts, docs) and update .github/templates/*-general.md AND *-template.md to reflect battle-tested conventions. Use when: after completing multiple phases, before starting a new project, whenever templates feel stale."
---
...
> **This prompt does NOT modify project-specific files** (`LEARN/{PROJECT}/GOAL.md`, `LEARN/{PROJECT}/NewAgent.md`). It only updates the universal `.github/templates/` files that feed into `/init` for future projects.
...
Read `.github/MANUAL.md`. Update it to reflect the current state of ALL prompts:
